...
class StudyGuideApp {
  ...
  renderGrades(subjectName) {
    ...
    this.dom.gradeList.innerHTML = grades
      .map(
        (grade) => `
  ...
  renderTopics(subjectName, grade) {
    if (!this.dom.topicList || !this.studyData?.subjects[subjectName]?.[grade])
      return;
  ...
  renderWelcomeScreen(type, subjectName, grade) {
    ...
    const { icon, color, title, message } =
      templates[type]?.() || templates.default();
  ...
  selectSubject(subjectName) {
    ...
    this.dom.topicList.innerHTML =
      '<p class="text-muted">Select a grade to see topics</p>';
  ...
  selectGrade(grade) {
    if (
      !this.currentSubject ||
      !this.studyData?.subjects[this.currentSubject]?.[grade]
    )
      return;
  ...
  selectTopic(topicName) {
    ...
    const topic =
      this.studyData.subjects[this.currentSubject][this.currentGrade][
        topicName
      ];
    if (!topic) return;
  ...
  updateSubjectDisplay(subjectName) {
    ...
    const { icon, color } = this.config.subjects[subjectName] || {
      icon: "bi-book",
      color: "text-secondary",
    };
  ...
  setupEventListeners() {
    ...
    document.querySelectorAll("[data-subject]").forEach((link) => {
      link.addEventListener("click", (e) => {
        ...
        if (this.dom.subjectSelect)
          this.dom.subjectSelect.value = link.dataset.subject;
      });
    });
  }
