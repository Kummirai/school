...
function calculateFraction(operation) {
  const num1 = parseInt(
    document.getElementById(operation === "add" ? "addNum1" : "multNum1").value
  );
  const den1 = parseInt(
    document.getElementById(operation === "add" ? "addDen1" : "multDen1").value
  );
  const num2 = parseInt(
    document.getElementById(operation === "add" ? "addNum2" : "multNum2").value
  );
  const den2 = parseInt(
    document.getElementById(operation === "add" ? "addDen2" : "multDen2").value
  );
...
// Modify the validateFractionAnswers function
function validateFractionAnswers(setId, answers) {
  ...
  if (feedbackDiv) {
    if (allCorrect) {
      ...
      feedbackDiv.textContent = `You got ${correctCount}/${answers.length} correct (${percentage}%)`;
